...
function check()
    ...
    if (numberOne-numberTwo) == input.getNumber() then
        timers[#timers + 1] = timer.performWithDelay( (delayTime + 1000 + (numberOne-numberTwo) * stepTime), function (event)
    ...
    local userUpdate = [[ UPDATE users SET lesson7 = ']] .. textData ..[[' WHERE id = ']] .. currentID .. [['; ]]
    db:exec( userUpdate )
...
function scene:create( event )
    ...
    for row in db:nrows("SELECT * FROM users") do
        ...
        local lessonData = cipher:decrypt ( row.lesson7, "sbs_math_key" )  
